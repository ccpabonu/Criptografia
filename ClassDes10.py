...
class ClassDes10:
    def __init__(self):
        # Mensaje
        self.textBin = "10111101"  # =M
        self.IPm = [1, 5, 2, 0, 3, 7, 4, 6]

        # Llaves
        self.keyBin = ['1', '0', '1', '0', '0', '0', '0', '0', '1', '0']  # k
    ...
    def encriptar(self, text):
        self.C = self.cifrado(text, self.keys)

    def desencriptar(self, text):
        self.keys.reverse()
        self.D = self.cifrado(text, self.keys)
        self.keys.reverse()

    def cifrado(self, text, keys):
        listM = list(text)
        save = []
        for i in self.IPm:
            save.append(listM[i])
