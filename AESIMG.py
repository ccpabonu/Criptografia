            ...
            for pixel in row:
                ...
                for component in pixel:
                    if len(three) < 48:
                        ...
                        first = self.f.Add(np.reshape(three[0: 16], (4, 4)),
                                           np.reshape(self.crip.encriptar(), (4, 4))).flatten()
                        temp[3][3] = self.f.Add(temp[3][3], 1)
                        self.crip.data = temp
                        second = self.f.Add(np.reshape(three[16: 32], (4, 4)),
                                            np.reshape(self.crip.encriptar(), (4, 4))).flatten()
                        temp[3][3] = self.f.Add(temp[3][3], 1)
                        self.crip.data = temp
                        temp = np.reshape(self.crip.encriptar(), (4, 4))
                        third = self.f.Add(np.reshape(three[32: 48], (4, 4)),
                                           np.reshape(self.crip.encriptar(), (4, 4))).flatten()
                        temp[3][3] = self.f.Add(temp[3][3], 1)
                        self.crip.data = temp
                        for k in range(5):
            ...
            for pixel in row:
                ...
                for component in pixel:
                    if len(three) < 6:
                        ...
                        self.crip.data = three
                        self.crip.setObject()
                        first = self.crip.encriptar()
                        print('es', len(first), first)
                        for k in range(2):
                            self.mat[self.i][self.j] = [first[0][3 * k], first[0][3 * k + 1], first[0][3 * k + 2]]
                            if self.j == self.col - 1:
                                self.j = 0
                                self.i += 1
                            else:
